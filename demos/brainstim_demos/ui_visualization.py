...
class MyThread(QThread):
    ...
    def run(self):     
        p = subprocess.Popen(['python','USTChopin_offline.py', '--model', self.model], stdout=subprocess.PIPE, stderr=subprocess.PIPE) # 通过成员变量传参
...
class MyThread_1(QThread):
    ...
    def run(self):     
        print("run")
        p = subprocess.Popen(['python','USTChopin_Stim.py', '--ip', self.ip, '--port',self.port], stdout=subprocess.PIPE, stderr=subprocess.PIPE) # 通过成员变量传参
        # try:
