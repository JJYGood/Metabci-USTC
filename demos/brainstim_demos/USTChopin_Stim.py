...
from metabci.brainflow.amplifiers import NeuracleServerThread
...
thread_data_server = NeuracleServerThread(threadName='data_server', device=target_device['device_name'], n_chan=target_device['n_chan'],
...
path = './demos/brainstim_demos/Stim_images/'
...
def display_text_image(text,fonts,image_path, color, screen):
    ...
    rect = text_surface.get_rect()
    rect.center = (x+image_width/2, 200) #左上角为原点，向右为x正方向，向下为y轴正方向
    screen.blit(text_surface, rect)   

    #更新显示
    pygame.display.flip()

def display_images_in_corners(screen):
    image_paths = [path+'1.jpg', path+'2.jpg', path+'3.jpg', path+'4.jpg']
    images = [pygame.image.load(path) for path in image_paths]
    image_rects = [image.get_rect() for image in images]
    image_widths = [rect.width for rect in image_rects]
    image_heights = [rect.height for rect in image_rects]
    
    # 定义四个角的位置
    positions = [
        (0, 0),  # 左上角
        (current_width - image_widths[0], 0),  # 右上角
        (0, current_height - image_heights[0]),  # 左下角
        (current_width - image_widths[0], current_height - image_heights[0])  # 右下角
    ]
    ...
    # 绘制图像在随机位置
    for image, pos in zip(images, positions):#random_positions
...
# 设置实验参数
num_repeats = 4  # 总试次数 = num_repeats * trials
rest_time = 2    # 休息时间（秒）
...
for repeat in range(num_repeats):
    ...
    for task in tasks:
        ...
        try:
            while not flagstop: # get data in one second step   
                # s_com.write([6])
                data = thread_data_server.get_bufferData()[:,-1600:]
                flagstop = True
        except:
            pass
        ...
        image = 'L'+str(task)+'R'+str(task)
        display_text_image('KEY'+" " +str(task),font_Chinese,image, WHITE, screen)
        display_images_in_corners(screen)
        marker1 = task 
        marker2 = 2   
        # s.write([task])
        # outlet.push_sample([task])
